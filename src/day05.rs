...
mod day05 {
     ...
     */
    fn largest_grid_product(vec: Vec<Vec<i32>>) -> i32 {
        let x_length = vec.get(0).unwrap().len();
        let y_length = vec.len();
        ...
        for y in 0..y_length {
            for x in  0..x_length {
                if x_length - x > 4 {
                    let horizontal = vec[y][x] * vec[y][x+1] * vec[y][x+2] * vec[y][x+3];
                    if result < horizontal {result = horizontal}
                }
                if y_length - y > 4 {
                    let vertical = vec[y][x] * vec[y+1][x] * vec[y+2][x] * vec[y+3][x];
                    if result < vertical {result = vertical}
                }
                if y_length - y > 4 && x_length - x > 4 {
                    let diagonally = vec[y][x] * vec[y+1][x+1] * vec[y+2][x+2] * vec[y+3][x+3];
                    if result < diagonally {result = diagonally}
                }
                if x > 2 && y_length - y > 4 {
                    let rev_diagonally = vec[y][x] * vec[y+1][x-1] * vec[y+2][x-2] * vec[y+3][x-3];
                    if result < rev_diagonally {result = rev_diagonally}
                }
            }
        }
        result
    ...
    fn problem11_test() {
        ...
        assert_eq!(largest_grid_product(example_grid), 70600674);
